/*
    В конец добавлен свой элемент выбора страниц.
    Он создается в строке 3214.
*/
        ...
        */
        function classSettings() {
            ...
            */
            ...
            //this.pageSelector = new PageSelector(oInit.pageSelectorParams);
        ...
        */
        function _fnDraw(oSettings) {
            ...
            */
            ...
            oSettings.oInit.pageSelectorParams.sInfoEmpty = oSettings.oLanguage.sInfoEmpty;

            if (oSettings.oInit.pageSelectorParams) {
                var pageSelector = new PageSelector(oSettings.oInit.pageSelectorParams);
            }
        }
...
/*
    "Элемент выбора страниц заменяет собой родной paginator в DataTables, что позволяет использовать их
    для отображения больших объемов данных.
*/
var PageSelector = function (oParams) {
    this.iTotal = oParams.iTotal;           //общее число записей
    this.iStart = oParams.iStart;           //начальный индекс
    this.iStep = oParams.iStep;             //шаг
    this.sParent = oParams.sParent;         //id таблицы, на которую навешивается DataTables
    this.fnOnSelect = oParams.fnOnSelect;   //колбэк при смене страницы (обычно пересоздание таблицы)
    this.oLanguage = oParams.oLanguage;
    this.sInfoEmpty = oParams.sInfoEmpty;

    this.aoData = oParams.data;
    this.asFields = oParams.fields;
    this.aiIndexes = oParams.indexes;
    this.sPreviousText = oParams.previousText;

    this.fnInit();
}

with(PageSelector){
    prototype.iEnd = 0;                     //конечный номер (не включительно)
}

PageSelector.prototype.fnInit = function () {
    var oPageSelector = this;

    var nFirst, nPrevious, nNext, nLast, nMessage, sFirstHTML, sPreviousHTML, sNextHTML, sLastHTML, sMessageHTML;

    /*
    4 кнопки: первая, назад, далее, последняя
    */

    sFirstHTML = "<span id='" + oPageSelector.sParent + "_first' ></span>";

    sLastHTML = "<span id='" + oPageSelector.sParent + "_last' ></span>";

    sPreviousHTML = "<span id='" + oPageSelector.sParent + "_previous' ></span>";
    sNextHTML = "<span id='" + oPageSelector.sParent + "_next' ></span>";

    sMessageHTML = "<label id='" + oPageSelector.sParent + "_message' ></label>";

    oPageSelector.iEnd = oPageSelector.iStart + oPageSelector.iStep;
    oPageSelector.iEnd = oPageSelector.iEnd > oPageSelector.iTotal ?
                oPageSelector.iTotal : oPageSelector.iEnd;

    //создаем кнопки
    nFirst = $(sFirstHTML);
    nPrevious = $(sPreviousHTML);
    nNext = $(sNextHTML);
    nLast = $(sLastHTML);
    nLabel = $(sMessageHTML);
    nLabel.text(oPageSelector.fnGetMessage());

    nFirst.append(oPageSelector.oLanguage.sFirst);
    nFirst.addClass("fg-button ui-button ui-state-default first ui-corner-tl ui-corner-bl");
    nLast.append(oPageSelector.oLanguage.sLast);
    nLast.addClass("fg-button ui-button ui-state-default last ui-corner-tr ui-corner-br");
    nPrevious.append(oPageSelector.oLanguage.sPrevious);
    nPrevious.addClass("fg-button ui-button ui-state-default previous");
    nNext.append(oPageSelector.oLanguage.sNext);
    nNext.addClass("fg-button ui-button ui-state-default next");

    var nFilter = $("#" + oPageSelector.sParent + "_filter");
    nFilter.empty();
    nFilter.append(oPageSelector.fnGetControl());

    //убираем родной селект числа отображаемых записей, чтобы не мешался
    var nLength = $("#" + oPageSelector.sParent + "_length");
    nLength.empty();
    if (oPageSelector.oLanguage.sTitle)
        nLength.append(oPageSelector.oLanguage.sTitle);

    //затираем родную информацию об отображаемой группе и пихаем свою
    var nInfo = $("#" + oPageSelector.sParent + "_info");
    nInfo.empty();
    nInfo.append(nLabel);

    //затираем свои кнопки и пихаем свои
    var nPaginate = $("#" + oPageSelector.sParent + "_paginate");
    nPaginate.empty();
    nPaginate.append(nFirst, nPrevious, nNext, nLast);
    /* Take the brutal approach to cancelling text selection */
    $('span', nPaginate)
		.bind('mousedown', function () { return false; })
		.bind('selectstart', function () { return false; });

    //навешиваем события на нажантия кнопок
    //таблица будет пересоздаваться, только если выбранные индексы реально изменились
    nFirst.click(function () {
        if (oPageSelector.fnPageChange("first")) {
            //nLabel.text(oPageSelector.fnGetMessage());
            
            $("#" + oPageSelector.sParent + "_message").text(oPageSelector.fnGetMessage())
            oPageSelector.fnOnSelect(oPageSelector.aiIndexes, oPageSelector.sPreviousText, oPageSelector.iStart, oPageSelector.iEnd, oPageSelector.iTotal);
        }
    });

    nPrevious.click(function () {
        if (oPageSelector.fnPageChange("previous")) {
            //nLabel.text(oPageSelector.fnGetMessage());
            $("#" + oPageSelector.sParent + "_message").text(oPageSelector.fnGetMessage())
            oPageSelector.fnOnSelect(oPageSelector.aiIndexes, oPageSelector.sPreviousText, oPageSelector.iStart, oPageSelector.iEnd, oPageSelector.iTotal);
        }
    ...
    nNext.click(function () {
        if (oPageSelector.fnPageChange("next")) {
            //nLabel.text(oPageSelector.fnGetMessage());
            $("#" + oPageSelector.sParent + "_message").text(oPageSelector.fnGetMessage())
            oPageSelector.fnOnSelect(oPageSelector.aiIndexes, oPageSelector.sPreviousText, oPageSelector.iStart, oPageSelector.iEnd, oPageSelector.iTotal);
        }
    ...
    nLast.click(function () {
        if (oPageSelector.fnPageChange("last")) {
            //nLabel.text(oPageSelector.fnGetMessage());
            $("#" + oPageSelector.sParent + "_message").text(oPageSelector.fnGetMessage())
            oPageSelector.fnOnSelect(oPageSelector.aiIndexes, oPageSelector.sPreviousText, oPageSelector.iStart, oPageSelector.iEnd, oPageSelector.iTotal);
        }
...
//возвращает сообщение об отображаемой группе элементов
PageSelector.prototype.fnGetMessage = function () {
    oPageSelector = this;
    var asInfo = this.oLanguage.asInfo;
    var sResult = asInfo[0] + String(oPageSelector.iStart + 1) + asInfo[1] + String(oPageSelector.iEnd) + asInfo[2] +
        String(oPageSelector.iTotal) + asInfo[3];
    if (oPageSelector.iTotal === 0)
        sResult = this.sInfoEmpty;
    return sResult;
}

/*
    изменяем выбранные индексы
    sAction - действие: first, previous, next или last
*/
PageSelector.prototype.fnPageChange = function(sAction){
    var oPageSelector = this;
    var iOldStart = oPageSelector.iStart;
    switch(sAction){
        case "first":
            oPageSelector.iStart = 0;
            oPageSelector.iEnd = oPageSelector.iStart + oPageSelector.iStep;
            
            //если конечный индекс больше чем "всего", кидаем туда "всего"
            oPageSelector.iEnd = oPageSelector.iEnd > oPageSelector.iTotal ?
                oPageSelector.iTotal : oPageSelector.iEnd;
            break;
        case "previous":
            oPageSelector.iStart = oPageSelector.iStart - oPageSelector.iStep;
            oPageSelector.iEnd = oPageSelector.iStart + oPageSelector.iStep;
            oPageSelector.iEnd = oPageSelector.iEnd >= oPageSelector.iTotal ?
            oPageSelector.iTotal : oPageSelector.iEnd;									
			
			//если индекс стал отрицательным, исправляем на 0
			if ( oPageSelector.iStart < 0 )
			{
			  oPageSelector.iStart = 0;
			  oPageSelector.iEnd = oPageSelector.iStart + oPageSelector.iStep;
                oPageSelector.iEnd = oPageSelector.iEnd >= oPageSelector.iTotal ?
			...
			break;
		case "next":		    
			/* Меняем индексы, только если мы не вылезем за границы */
			if ( oPageSelector.iStart + oPageSelector.iStep < oPageSelector.iTotal )
			{
				oPageSelector.iStart += oPageSelector.iStep;
				oPageSelector.iEnd = oPageSelector.iStart + oPageSelector.iStep;
				//на всякий случай
                oPageSelector.iEnd = oPageSelector.iEnd >= oPageSelector.iTotal ?
                oPageSelector.iTotal : oPageSelector.iEnd;
			...
			break;
		case "last":
		    //вдруг кто захочет сделать шаг 0
		    if ( oPageSelector.iStep >= 0 )
			{
				var iPages = parseInt( (oPageSelector.iTotal - 1) / oPageSelector.iStep, 10 ) + 1;
				oPageSelector.iStart = (iPages-1) * oPageSelector.iStep;
				oPageSelector.iEnd = oPageSelector.iStart + oPageSelector.iStep;
                    oPageSelector.iEnd = oPageSelector.iEnd >= oPageSelector.iTotal ?
                    oPageSelector.iTotal : oPageSelector.iEnd;
			}
    ...
    return iOldStart != oPageSelector.iStart;
}

//возвращает полный массив индексов для поиска
PageSelector.prototype.fnGetDefaultIndexes = function(){
    var aiIndexes = [];
...
/*
    После окончания поиска массив aiIndexes будет содержать индексы объектов,
    в которых найдено вхождение sValue
*/
PageSelector.prototype.fnProcessSearch = function(sValue){
    var aoData = this.aoData;                       //массов объектов, в котором осуществляется поиск
    var asFields = this.asFields;                   //поля объекта, по которым осуществляется поиск
    var aiIndexes = this.fnGetDefaultIndexes();     //строим массив индексов
    var i=0;
    while (i<aiIndexes.length){
        var notFound = true;
        //просматриваем все поля
        for (var j=0; j<asFields.length; j++){
            if (String(aoData[aiIndexes[i]][asFields[j]]).toUpperCase().search(sValue.toUpperCase())!=-1){
                notFound = false;
            }
        }
        if (notFound){
            //если sValue в объекте не найдено, удалаем его индекс из массива
            aiIndexes.splice(i,1);
        }
        else{
            //если найдено, переходим к следующему объекту
            i++;
        }
    }
...
//возвращает контрол для поиска
PageSelector.prototype.fnGetControl = function(){
    var oPageSelector = this;
    var nFilterControl = $("<img src='/Website/images/sk.gif' height=16px width=16px/ title='"+ oPageSelector.oLanguage.sFilterTooltip +"'> <input type='text'/>");
    nFilterControl.val(this.sPreviousText);
    //вешаем событие на изменение
    nFilterControl.change(function(){
        if ($(this).val()!=''){
            oPageSelector.fnProcessSearch($(this).val());
            oPageSelector.fnOnSelect(oPageSelector.aiIndexes, $(this).val(), 0, oPageSelector.iStep, oPageSelector.aiIndexes.length);
        }
        else{
            //если строка поиска пустая, показываем все объекты
            oPageSelector.aiIndexes = oPageSelector.fnGetDefaultIndexes();
            oPageSelector.fnOnSelect(oPageSelector.aiIndexes, $(this).val(), 0, oPageSelector.iStep, oPageSelector.aiIndexes.length);
        }
    });
    nFilterControl.find('img').click(function(){
        if ($(this).val()!=''){
            oPageSelector.fnProcessSearch($(this).val());
            oPageSelector.fnOnSelect(oPageSelector.aiIndexes, $(this).val(), 0, oPageSelector.iStep, oPageSelector.aiIndexes.length);
        }
    ...
    nFilterControl.find('input').addClass("dataTables_filter");
    AddHeadenText(nFilterControl,oPageSelector.oLanguage.searchText);
    return nFilterControl;
}