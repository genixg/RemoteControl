...
;(function($){
	$.contextMenu = {
		...
		beforeShow: function() { 
		document.lastActiveElement = document.activeElement;
		document.clipboard = getSelText(); return true; 
		},
	...
	$.fn.contextMenu = function(menu,options,aFunc, copypaste) {
		if(copypaste) {		
			menu.push( {'Копировать текст':{
				onclick: function(y) { if(document.clipboard) window.clipboardData.setData('Text', document.clipboard ); },
				icon: "/Website/images/copy.png" }}
			); 	
			menu.push( {'Вставить текст':{
				onclick: function(y) {
					if(window.clipboardData && document.lastActiveElement && typeof(document.lastActiveElement.value)!="undefined")
						document.lastActiveElement.value = window.clipboardData.getData('Text');
					},
				icon: "/Website/images/paste.png" }}
			); 
		}
		var cmenu = $.contextMenu.create(menu,options);
		this.each(function(){
			$(this).bind('contextmenu',function(e){if(aFunc!=undefined){aFunc(cmenu, this);};cmenu.show(this,e);return false;});
		});
		return cmenu;
	};
	
	// навязка существующего меню на строки
	$.fn.addContexMenu = function(cmenu,aFunc) {
	    this.each(function(){
			$(this).bind('contextmenu',function(e){if(aFunc!=undefined){aFunc(cmenu, this);};cmenu.show(this,e);return false;});
		});
	}
